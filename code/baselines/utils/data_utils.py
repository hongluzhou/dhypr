...
def load_graph(filepath):
    ...
    return G, adj, train_edges, val_edges, val_edges_false, test_edges, test_edges_false, node_str2int
...
def load_data(args):
    if args.task == 'nc':
        ...
        if args.task == 'lp':
            G, adj_train, train_edges, val_edges, val_edges_false, test_edges, test_edges_false, node_str2int = \
            ...
            x, y = sp.triu(sp.csr_matrix(1. - adj_train.toarray())).nonzero()
            train_edges_false = np.array(list(zip(x, y)))
